...
class AstroMessageSwipeController(private val context: Context, private val swipeControllerActions: AstroSwipeControllerActions) :
    ...
    override fun getMovementFlags(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder): Int {
        ...
        imageDrawable = ContextCompat.getDrawable(context,R.drawable.at_ic_reply_black_24dp)!!
        shareRound = ContextCompat.getDrawable(context, R.drawable.at_ic_round_shape)!!
    ...
    private fun setTouchListener(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) {
        recyclerView.setOnTouchListener { _, event ->
            ...
            if (swipeBack) {
                if (abs(mView.translationX) >= this@AstroMessageSwipeController.convertToDp(100)) {
    ...
    private fun drawReplyButton(canvas: Canvas) {
        ...
        val dt = 17.coerceAtMost((newTime - lastReplyButtonAnimationTime).toInt())
        ...
        if (showing) {
            ...
            alpha = 255f.coerceAtMost(255 * (replyButtonProgress / 0.8f)).toInt()
        } else {
            scale = replyButtonProgress
            alpha = 255f.coerceAtMost(255 * replyButtonProgress).toInt()
        }
        ...
        shareRound.colorFilter =
            PorterDuffColorFilter(ContextCompat.getColor(context, R.color.at_grey7), PorterDuff.Mode.MULTIPLY)
    ...
    private fun convertToDp(pixel: Int): Int {
        return AstroAndroidUtils.dp(pixel.toFloat(), context)
    }
}
