...
import com.astrotalk.sdk.api.activities.UserAstrologerChatWindowActivity;
...
public class ChatAstrologerListAdapter extends RecyclerView.Adapter<ChatAstrologerListAdapter.ViewHolder> {
    ...
    private int serviceId = Constants.CHAT_SERVICE_ID;
    private RequestQueue requestQueue;

    public ChatAstrologerListAdapter(Activity context, ArrayList<UniversalAstrologerListModel> astrologerListModelArrayList) {
    ...
    @Override
    public void onBindViewHolder(@NonNull final ChatAstrologerListAdapter.ViewHolder holder, @SuppressLint("RecyclerView") final int position) {
        ...
        if (userChatListModel.getHasOffer() && userChatListModel.getCashbackOfferValue() != 0) {
            ...
            if (Constants.TIME_ZONE.equalsIgnoreCase("Asia/Calcutta") || Constants.TIME_ZONE.equalsIgnoreCase("Asia/Kolkata")) {
                if (userChatListModel.isPo()) {
                    ...
                    holder.price2.setVisibility(View.VISIBLE);
                    holder.price.setText(Utilities.getConvertedValueFromINR((userChatListModel.getPrice() - userChatListModel.getCashbackOfferValue()), sharedPreferences));
                }
            } else {
                if (userChatListModel.isPo()) {
                    holder.price.setText("FREE");
        ...
        if (userChatListModel.getProfilePic().trim().isEmpty()) {
            ...
            Glide.with(context).load(Constants.IMAGE_DOMAIN + userChatListModel.getProfilePic().trim()).error(R.drawable.at_ic_bg_astrologer).placeholder(R.drawable.at_ic_bg_astrologer).into(holder.userPicIV);
        }
